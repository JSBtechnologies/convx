...
- Linux: run dependency/bootstrap wrapper via [installers/linux/run-installer.sh](linux/run-installer.sh)
...
- Linux: [installers/bootstrap-linux.sh](bootstrap-linux.sh)
...
- Linux run wrapper: [installers/linux/run-installer.sh](linux/run-installer.sh)
...
- `CONVX_INSTALLER_LOG` — override installer/bootstrap log path
	- default: `/tmp/convx-installer.log` on macOS/Linux
...
`bash installers/bootstrap-linux.sh`
...
- shows license acceptance during installer flow
