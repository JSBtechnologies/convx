    ...
    match name {
        ...
        "convert_file" => {
            ...
            let input = match (&p.input_path, &p.input_content) {
                ...
                (None, None) => {
                    return Err("Either input_path or input_content is required".to_string())
                }
            };
            let preset = p
                .preset
        ...
        "batch_convert" => {
            ...
            let input_paths: Vec<PathBuf> = match (&p.input_paths, &p.input_contents) {
                (Some(paths), _) if !paths.is_empty() => paths.iter().map(PathBuf::from).collect(),
                (_, Some(contents)) if !contents.is_empty() => {
                    ...
                    if filenames.len() != contents.len() {
                        return Err(
                            "input_contents and input_filenames must have the same length"
                                .to_string(),
                        );
                    }
                ...
                _ => {
                    return Err(
                        "Either input_paths or input_contents must be provided and non-empty"
                            .to_string(),
                    )
                }
            };
...
fn save_base64_to_temp(content: &str, filename: &str) -> Result<PathBuf, String> {
    ...
    let safe_name = std::path::Path::new(filename)
        .file_name()
        .ok_or_else(|| format!("invalid input_filename: {:?}", filename))?;
