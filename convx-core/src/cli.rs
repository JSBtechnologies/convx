use crate::{
    license, presets, ConversionOptions, ConvxEngine, DependencyChecker, FfprobeInfo, Format,
...
struct Cli {
    ...
    /// Skip license check (dev builds only)
...
enum Commands {
    ...
    Activate {
        /// License key (e.g. CONVX-XXXX-XXXX-XXXX-XXXX)
        key: String,
    },
    ...
    /// Show device fingerprint (debug/support)
    Fingerprint,
...
pub fn cli_main() -> anyhow::Result<()> {
    ...
    let skip_license = cli.skip_license;
    let engine = ConvxEngine::new();

    // Commands that require a valid license
    let needs_license = matches!(
        cli.command,
        Commands::Convert { .. } | Commands::Watch { .. } | Commands::Mcp
    );

    if needs_license && !skip_license {
        if let Err(msg) = license::require_license() {
            if json_output {
                let payload = serde_json::json!({ "error": "license_required", "message": msg });
    ...
    match cli.command {
        ...
        Commands::Activate { key } => match license::activate(&key) {
            Ok(license::ActivateOutcome::Activated { device_name }) => {
                if json_output {
                    let payload = serde_json::json!({
                        "status": "activated",
                        "device": device_name,
                    ...
                    println!("✓ License activated on: {}", device_name);
                    println!("  You're all set. Run `convx convert` to get started.");
                }
            }
            Ok(license::ActivateOutcome::AlreadyActive { device_name }) => {
                if json_output {
                    let payload = serde_json::json!({
                    ...
                    println!("This license is currently active on: \"{}\"\n", device_name);
                    print!("Would you like to transfer it to this device? [y/N] ");
                    use std::io::Write;
                    std::io::stdout().flush()?;
                    ...
                    {
                        match license::transfer(&key) {
                            Ok(()) => {
                                println!("✓ License transferred to this device.");
                            }
                            Err(e) => {
        ...
        Commands::Deactivate => {
            if !json_output {
                print!("This will deactivate ConvX on this device. Continue? [y/N] ");
                use std::io::Write;
                std::io::stdout().flush()?;
            ...
            match license::deactivate() {
                Ok(()) => {
                    if json_output {
                        ...
                        println!("✓ License deactivated. You can re-activate on another device.");
                    }
                }
                Err(e) => {
                    if json_output {
        ...
        Commands::License => match license::license_info() {
            Some(info) => {
                if json_output {
                    let payload = serde_json::json!({
                        "status": "active",
                    ...
                    println!("License:    {}", info.key_masked);
                    println!("Device:     {}", info.device_name);
                    println!("Platform:   {}", info.platform);
                    println!("Activated:  {}", info.activated_at.format("%Y-%m-%d"));
                    println!("Next check: {}", info.recheck_after.format("%Y-%m-%d"));
                }
            }
            None => {
                if json_output {
                    let payload = serde_json::json!({ "status": "not_activated" });
                    println!("{}", serde_json::to_string_pretty(&payload)?);
                } else {
                    println!("ConvX is not activated.");
                    println!("Run `convx activate <KEY>` to get started.");
                    println!("Purchase a license at https://convx.dev");
                }
            }
        },

        Commands::Fingerprint => match license::fingerprint_debug() {
            Ok(debug) => {
                if json_output {
                    println!("{}", serde_json::to_string_pretty(&debug)?);
                } else {
                    println!(
                        "Device Fingerprint (schema v{})",
                        debug.fingerprint.schema_version
                    );
                    println!("═══════════════════════════════════════");
                    println!("Device ID:   {}", &debug.fingerprint.device_id[..16]);
                    println!("Device Name: {}", debug.fingerprint.device_name);
                    println!("Platform:    {}", debug.fingerprint.platform);
                    println!("Tier 1 hash: {}", &debug.fingerprint.tier1_hash[..16]);
                    println!("Tier 2 hash: {}", &debug.fingerprint.tier2_hash[..16]);
                    println!();
                    println!("Collected Signals (redacted)");
                    println!("───────────────────────────────────────");
                    println!("Platform UUID:    {}", debug.signals.platform_uuid);
                    println!("CPU ID:           {}", debug.signals.cpu_id);
                    println!("CPU Microarch:    {}", debug.signals.cpu_microarch);
                    println!("TPM Hash:         {}", debug.signals.tpm_hash);
                    println!("Board Serial:     {}", debug.signals.board_serial);
                    println!("Machine ID:       {}", debug.signals.machine_id);
                    println!("Model:            {}", debug.signals.model);
                    println!("Total RAM:        {}", debug.signals.total_ram);
                    println!("GPU ID:           {}", debug.signals.gpu_id);
                    println!("Chassis Serial:   {}", debug.signals.chassis_serial);
                    println!("Firmware Version: {}", debug.signals.firmware_version);
                    println!("Hostname:         {}", debug.signals.hostname);
                    println!("OS Version:       {}", debug.signals.os_version);
                    println!("Boot Disk UUID:   {}", debug.signals.boot_disk_uuid);
                }
            }
            Err(e) => {
                if json_output {
                    ...
                    eprintln!("Failed to collect fingerprint: {}", e);
                }
                std::process::exit(1);
            }
        },
