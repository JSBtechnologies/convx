use convx::{
    silent_command, ConversionOptions, ConversionStatus, ConvxEngine, DependencyChecker, Format,
    FormatCategory,
};
...
pub struct ConvxState {
    ...
    /// Active conversion's cancel flag; None when no conversion is running.
...
fn resolve_existing_path(path: &str) -> Result<PathBuf, String> {
    ...
    let canonical =
        dunce::canonicalize(&raw).map_err(|e| format!("Invalid path {}: {}", raw.display(), e))?;
...
pub async fn convert_file(
    ...
    let convert_result = state.engine.convert_with_progress(
        input_path.as_path(),
        output_path.as_path(),
        conv_options,
        &mut on_progress,
    ...
    let result = convert_result.map_err(|e| {
        let _ = window.emit(
            "conversion-progress",
            JsConversionProgress {
...
pub fn install_dependencies() -> JsDependencyStatus {
    let missing_categories = get_missing_dependencies();
    ...
    for category in &missing_categories {
        let result = install_single_dependency(category.clone());
        if !result.ok {
            errors.push(result.message);
        }
    }
...
pub fn auto_configure_mcp(target: String) -> Result<String, String> {
    ...
    let json_bytes = serde_json::to_string_pretty(&config).unwrap_or_else(|_| "{}".to_string());
    ...
    if std::fs::rename(&tmp_path, &config_path).is_err() {
        ...
        std::fs::write(&config_path, &json_bytes).map_err(|e| {
            format!(
