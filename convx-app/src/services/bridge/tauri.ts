...
export class TauriBridge {
  ...
  async activateLicense(key: string): Promise<ActivateResult> {
    return invoke<ActivateResult>('activate_license', { key });
  }

  async transferLicense(key: string): Promise<boolean> {
    return invoke<boolean>('transfer_license', { key });
  }

  async deactivateLicense(): Promise<boolean> {
    return invoke<boolean>('deactivate_license');
  }
  ...
  async getEnterpriseConfig(): Promise<EnterpriseConfig> {
    return invoke<EnterpriseConfig>('get_enterprise_config');
  }

  async autoActivate(): Promise<ActivateResult> {
    return invoke<ActivateResult>('auto_activate');
  }

  async sendConversionAudit(data: {
    inputFormat: string;
    outputFormat: string;
    inputSize: number;
    outputSize: number;
    ...
    await invoke('send_conversion_audit', {
