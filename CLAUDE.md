...
**convx** is an open-source, local-first file conversion tool (Apache-2.0) with three surfaces sharing a single Rust core:
...
- `commands.rs` — IPC command handlers (conversion, formats, dependencies, MCP config); includes path security validation blocking sensitive system roots
