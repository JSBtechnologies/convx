...
- **Three interfaces** — CLI, desktop app (macOS/Windows/Linux), and MCP server
...
**Debian / Ubuntu:**
```bash
sudo apt-get install ffmpeg libvips-tools pandoc poppler-utils libreoffice-core \
  libreoffice-writer libreoffice-calc libreoffice-impress python3 python3-venv
python3 -m venv ~/.convx/venv
~/.convx/venv/bin/pip install pandas openpyxl weasyprint pdf2docx "PyMuPDF==1.23.26" mobi pyarrow numpy h5py
```
...
npm run tauri:bundle     # Production build (.app / .exe / .deb)
```
...
│   │   ├── license/           Ed25519-signed device licensing
...
│   └── src-tauri/             Rust backend with 25+ IPC commands
├── license-server/        License management API
│   └── src/                   Cloudflare Workers + Supabase
...
Proprietary. Copyright &copy; 2025 JSB Technologies. All rights reserved. See [LICENSE](LICENSE).
